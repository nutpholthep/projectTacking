...
<body>
                    ...
                    <?php while ($lact = mysqli_fetch_assoc($act_query)) { ?>
                        ...
                        <?php if (isset($lact['activity_id'])) { ?>
                            <tr>
                                ...
                                <td>
                                    <a href="deleteActivity.php?idact=<?php echo $lact['activity_id'] ?>" class="btn btn-danger" onclick="confirmDelete(event,<?php echo $lact['activity_id'] ?>)">
                                        <i class="bi bi-trash"></i>ลบงาน
                                    </a>
                                </td>
    ...
    <script>
    function confirmDelete(event,actid) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire(
                    'ลบงาน!',
                    'คุณได้ทำการลบงานสำเร็จแล้ว.',
                    'success'
                ).then(() => {
                    // Redirect after the SweetAlert dialog is closed
                    window.location.href = "deleteActivity.php?idact="+actid+"";
                });
            }
        });
    }
